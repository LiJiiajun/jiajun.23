...
(* ramstyle = "M9K" *) logic [31:0] VRAM [0:`VRAM_WORDS-1]; // Registers
...
always_ff @(posedge CLK) begin
	if(RESET) begin
		...
		byte_id_q<=byte_id;
		visible_q<=visible;
		if(visible && (word_id<`VRAM_WORDS))
			draw_vram_word<=VRAM[word_id];
		else
			draw_vram_word<=32'h0000_0000;
		if(AVL_CS && AVL_WRITE && (AVL_ADDR<`VRAM_WORDS)) begin
			if(AVL_BYTE_EN[0]) VRAM[AVL_ADDR][7:0]<=AVL_WRITEDATA[7:0];
			if(AVL_BYTE_EN[1]) VRAM[AVL_ADDR][15:8]<=AVL_WRITEDATA[15:8];
			if(AVL_BYTE_EN[2]) VRAM[AVL_ADDR][23:16]<=AVL_WRITEDATA[23:16];
			if(AVL_BYTE_EN[3]) VRAM[AVL_ADDR][31:24]<=AVL_WRITEDATA[31:24];
		end
		else if(AVL_CS && AVL_WRITE && (AVL_ADDR==`CTRL_ADDR)) begin
		...
		if(AVL_CS && AVL_READ) begin
			if(AVL_ADDR<`VRAM_WORDS)
				AVL_READDATA<=VRAM[AVL_ADDR];
			else if(AVL_ADDR==`CTRL_ADDR)
				AVL_READDATA<=CTRL_REG_DATA;
			else
				AVL_READDATA<=32'h0000_0000;
		end
	end
